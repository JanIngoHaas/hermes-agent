...
def _get_backend() -> str:
    ...
    for provider in _list_registered_web_providers():
        if provider.name in _LEGACY_WEB_BACKENDS:
            continue
        try:
            # Must support SEARCH: this return value becomes the shared
            # backend, and _get_capability_backend("search") falls through
            # to it. An extract-only provider (e.g. trafilatura) winning
            # here makes web_search resolve to a backend that cannot serve
            # it. Extract-only providers stay reachable through an explicit
            # ``web.extract_backend`` selection, which is how they are meant
            # to be chosen.
...
# ─── Optional question-directed extraction (opt-in; off unless a caller
# passes question). The default web_extract path stays LLM-free. ────
...
async def process_content_with_llm(
    ...
    """
    Question-directed extraction from web content via a single fast-aux LLM call.

    Given the user's ``question``, the auxiliary model returns ONLY the
    information that answers it, dropping everything irrelevant, and appends a
    <=3-sentence note on what else the page covers. This replaces the old
    generic "summarize everything" pass, which was both slow (large model over
    the whole page) and lossy (no idea what the caller wanted).

    Single stable-prefix call only — no chunking, no multi-call retrieval loop.
    Oversized pages are truncated to ``WEB_EXTRACT_BUDGET_CHARS`` rather than
    chunked: any sequential/iterative pass reintroduces the latency and
    prompt-cache misses this design exists to avoid.
    ...
    Args:
        ...
        model (str): Override model; defaults to the auxiliary web_extract backend
        min_length (int): Skip processing below this length (default: 5000)
        question (str): What the caller wants from the page — drives extraction

    Returns:
        Optional[str]: Targeted markdown answer, or None if content too short or
        processing fails.
    """
    # ~25k tokens ≈ 100k chars (4 chars/tok). Larger pages are truncated to a
    # single-call budget; we never chunk or multi-call (see docstring).
    WEB_EXTRACT_BUDGET_CHARS = 100_000
    MAX_OUTPUT_SIZE = 8000  # Hard cap on the targeted answer + brief note

    try:
        content = content or ""
        content_len = len(content)

        # Skip processing if content is too short to be worth an LLM round-trip.
        if content_len < min_length:
            logger.debug("Content too short (%d < %d chars), skipping LLM processing", content_len, min_length)
            return None

        # Truncate oversized pages to a single-call budget (never chunk).
        truncated_note = ""
        if content_len > WEB_EXTRACT_BUDGET_CHARS:
            logger.info(
                "Content %d chars > budget %d — truncating for single-call extraction",
                content_len, WEB_EXTRACT_BUDGET_CHARS,
            )
            content = content[:WEB_EXTRACT_BUDGET_CHARS]
            truncated_note = (
                f"\n\n[NOTE: the original page was {content_len:,} chars; it has been "
                f"truncated to {WEB_EXTRACT_BUDGET_CHARS:,} chars for processing — some "
                f"later content is not shown.]"
            )
        ...
        logger.info(
            "Question-directed extraction (%d chars, q=%r)",
            len(content), (question or "")[:80],
        )
        processed_content = await _call_summarizer_llm(
            content + truncated_note, context_str, model, question=question
        )

        if processed_content:
            # Enforce output cap
            if len(processed_content) > MAX_OUTPUT_SIZE:
                processed_content = processed_content[:MAX_OUTPUT_SIZE] + "\n\n[... answer truncated for context management ...]"
...
async def _call_summarizer_llm(
    ...
    """
    Make a single question-directed extraction call against the page content.

    The model answers ONLY what ``question`` needs, omits everything irrelevant,
    and appends a <=3-sentence note on what else the page covers.

    Args:
        content: The page markdown to extract from
        context_str: Context information (title, URL)
        model: Model to use
        max_tokens: Maximum output tokens
        question: What the caller wants from the page

    Returns:
        Targeted markdown answer, or None on failure
    """
    question = (question or "").strip()
    question_block = question if question else "(No explicit question was provided — extract the page's main substance.)"

    system_prompt = """You are a precise extraction engine. You are given a web page (as markdown) and a QUESTION. Your only job is to answer the question from the page.

Rules:
1. Include ONLY information from the page that is relevant to the question. You MUST leave out everything on the page that does not bear on the question — navigation, unrelated sections, boilerplate, tangents — even if it is interesting. Do not pad.
2. Preserve specifics that matter to the question verbatim: exact figures, names, dates, quotes, code snippets, URLs, and tables, in their original formatting.
3. If the page does not contain the answer, say so plainly in one line rather than guessing. Never invent facts not present in the page.
4. After your answer, add a section exactly titled "## Rest of page (brief)" and, in NO MORE THAN 3 SENTENCES, very briefly note what else the page covers that you left out. Keep it terse — it is a pointer, not a summary.

Output clean markdown. Be targeted and tight."""

    user_prompt = f"""{context_str}QUESTION:
{question_block}

PAGE CONTENT (markdown):
{content}

Answer the QUESTION using only relevant information from the page, dropping everything irrelevant. Then add the "## Rest of page (brief)" section (max 3 sentences)."""
...
async def web_extract_tool(
    ...
    Args:
        ...
        question (str): Optional. When non-empty, each page additionally goes
            through a single question-directed auxiliary-model call that keeps
            only what answers the question plus a <=3-sentence note on the
            rest. Omitted (the default) means no LLM call at all — the fast,
            lossless truncate-and-store path above.
    ...
    try:
        ...
        # ─── Opt-in question-directed pass ──────────────────────────────
        # Only when the caller supplied a question. The default path above
        # is already complete and LLM-free; this layers one fast-aux call
        # per page on top, run over the FULL clean text (not the truncated
        # window) so the model sees everything within its own budget.
        if question and question.strip():
            if check_auxiliary_model():
                effective_model = _get_default_summarizer_model()
                logger.info("Question-directed extraction over %d page(s)...", pages_extracted)
                debug_call_data["processing_applied"].append("question_directed_extraction")

                async def _direct_single(result):
                    url = result.get("url", "")
                    if result.get("error"):
                        return result, None, "skipped"
                    ...
                    processed = await process_content_with_llm(
                        raw_content, url, title, effective_model,
                        DEFAULT_MIN_LENGTH_FOR_SUMMARIZATION, question=question,
                    )
                    if not processed:
                        # Too short, or the aux call failed — the truncate
                        # path's content already in `result` stands.
                        return result, None, "kept_raw"
                ...
                # return_exceptions=True: one bad page must not discard the rest.
                for item in await asyncio.gather(
                    *[_direct_single(r) for r in response.get("results", [])],
                    return_exceptions=True,
                ):
                    if isinstance(item, BaseException):
                        logger.warning("Question-directed extraction failed for a page: %s", item)
                        continue
                    _res, _metrics, _status = item
                    if _status == "processed":
                        debug_call_data.setdefault("compression_metrics", []).append(_metrics)
                        debug_call_data["pages_processed_with_llm"] = (
                            debug_call_data.get("pages_processed_with_llm", 0) + 1
                        )
                        logger.info("%s (question-directed)", _res.get("url", ""))
            else:
                logger.warning(
                    "question= was passed but no auxiliary model is available; "
                    "returning the unfiltered page content."
                )
                debug_call_data["processing_applied"].append("question_directed_unavailable")
...
WEB_EXTRACT_SCHEMA = {
    "name": "web_extract",
    "description": "Extract content from web page URLs. By default returns clean page content in markdown/text with no LLM summarization (fast). Also works with PDF URLs (arxiv papers, documents) — pass the PDF link directly. Pages within the char budget (default 15000) return whole; larger pages return a head+tail window with a footer telling you the full text's saved file path and the read_file call to page through the omitted middle. Inline images appear as [IMAGE: alt] placeholders; real image URLs are kept as links. Optionally pass `question` to get back only the parts of the page that answer it (costs one extra fast auxiliary-model call per page). If a URL fails or times out, use the browser tool instead.",
    "parameters": {
        "type": "object",
        "properties": {
            "urls": {
            ...
            "question": {
                "type": "string",
                "description": "Optional. What you want to learn from the page(s). When set, the extraction is targeted to it — irrelevant content is dropped and a <=3-sentence note describes what else the page covers. Costs one extra fast auxiliary-model call per page, so omit it when you just want the raw page. Be specific (e.g. 'What are the author's publications on web engineering?' rather than 'tell me about this page')."
            }
        },
        "required": ["urls"]
    }
