...
TOOL_CATEGORIES = {
    ...
    "web": {
        ...
        "providers": [
            ...
            # _write_provider_config(); post_setup installs both packages.
            {
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        try:
            ...
            _print_info("    Installing trafilatura (HTML→Markdown extraction package)...")
            try:
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    # Set web search/extract backend(s) in config if applicable. Supports both
    # the shared ``web_backend`` (1:1 provider rows) and the per-capability
    # ``web_search_backend`` / ``web_extract_backend`` fields used by preset
    # rows that pair a search-only and an extract-only backend (e.g.
    # "DuckDuckGo + Trafilatura"). Per-capability keys take precedence over the
    # shared ``web.backend`` at dispatch time.
    if provider.get("web_backend"):
    ...
    for _prov_key, _cfg_key in (
        ("web_search_backend", "search_backend"),
        ("web_extract_backend", "extract_backend"),
    ):
        if provider.get(_prov_key):
            _set_selection("web", _cfg_key, provider[_prov_key])
...
def _configure_provider(
    ...
    *,
    ...
    # Report the web search/extract backend(s); the actual persist happens
    # below via _write_provider_config (single source of truth).
    if provider.get("web_backend"):
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Set web search/extract backend(s) in config if applicable. Supports the
    # shared ``web_backend`` and the per-capability ``web_search_backend`` /
    # ``web_extract_backend`` fields used by preset rows (e.g. ddgs + trafilatura).
    if any(
        provider.get(k)
