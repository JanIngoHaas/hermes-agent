...
def test_write_provider_config_shared_web_backend():
    """A 1:1 provider row still writes the shared web.backend key."""
    config = {}
    _write_provider_config({"web_backend": "firecrawl"}, config, managed_feature=None)
    assert config["web"]["backend"] == "firecrawl"
    assert "use_gateway" not in config["web"]


def test_write_provider_config_per_capability_web_backend():
...
def test_write_provider_config_noop_without_web_fields():
    """No web-backend fields → no web config written."""
    config = {}
    _write_provider_config({"name": "Something Else"}, config, managed_feature=None)
...
def test_ddgs_trafilatura_preset_row_present_and_wires_both_keys():
    ...
    config = {}
    _write_provider_config(combo, config, managed_feature=None)
    assert config["web"]["search_backend"] == "ddgs"
    assert config["web"]["extract_backend"] == "trafilatura"
...
def test_apply_provider_selection_web_sets_backend():
    ...
    assert config["web"]["backend"] == "firecrawl"
    # Upstream dropped the use_gateway flag; the key must not be written.
    assert "use_gateway" not in config["web"]
